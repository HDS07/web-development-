...
function newGame(difficulty) {
  ...
  solution = solveGrid(psNum, rows, true);              // solve the grid

  timer = 0;                                        // reset the game state timer and remaining number
...
function getGridInit() {
  ...
  for (var i = 0; i < rand.length; i++) {         // put numbers in the grid
...
function ViewPuzzle(grid) {                              // view grid in webpage
...
function startTimer() {
  var timerDiv = document.getElementById("timer");      // start stopwatch timer
  ...
  intervalId = setInterval(function () {
    if (!pauseTimer) {
      timer++;
      var min = Math.floor(timer / 60);      // update stopwatch value every one second
      var sec = timer % 60;
      timerDiv.innerText =
...
function checkButtonClick() {                      // check grid if correct
  if (gameOn) {
    timer += 60;
    var currentGrid = [];
...
function restartButtonClick() {                         // restart game
  if (gameOn) {
    for (var i in remaining) remaining[i] = 9;
    ViewPuzzle(puzzle);
    updateRemainingTable();
...
function SurrenderButtonClick() {                     // surrender
  if (gameOn) {
    for (var i in remaining) remaining[i] = 9;
    ViewPuzzle(solution);                                  // update remaining numbers table
    updateRemainingTable();
    gameOn = false;                                        // stop the game
    pauseTimer = true;                                    // mark game as solved
    clearInterval(intervalId);         
    document.getElementById("game-difficulty").innerText = "Solved";
  }
}
...
function sudokuSolverMenuClick() {      // sudoku solver section
  hideHamburgerClick();
  if (gameOn) {
    gameOn = false;
    clearInterval(intervalId);
  ...
  var grid = [];
  for (var i = 0; i < 9; i++) {               // generate empty grid
    grid.push("");
    for (var j = 0; j < 9; j++) {
      grid[i] += "0";
    }
...
function isUniqueButtonClick() {                    // check if gird is already solved
  if (!isSolved) {                                  // if not try to solve it
  ...
  var columns = getColumns(puzzle);              // solve it again but start from the end
  var blocks = getBlocks(puzzle);
